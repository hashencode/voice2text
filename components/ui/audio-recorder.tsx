...
export function AudioRecorder({
    ...
    const recordingOptions = customRecordingOptions || (quality === 'high' ? RecordingPresets.HIGH_QUALITY : RecordingPresets.LOW_QUALITY);
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                if (!status.granted) {
                    Alert.alert('Permission Required', 'Please grant microphone permission to record audio.', [{ text: 'OK' }]);
                }
            } catch (error) {
    ...
    if (!permissionGranted) {
        return (
            <View style={[styles.container, { backgroundColor: secondaryColor }, style]}>
                <TextX variant="body" style={{ color: textColor, textAlign: 'center' }}>
        ...
        <View style={[styles.container, { backgroundColor: secondaryColor }, style]}>
            {recordingUri && !isRecording ? (
                <View style={{ alignItems: 'center' }}>
                    <AudioPlayer source={{ uri: recordingUri }} showControls={true} showWaveform={true} showTimer={true} autoPlay={false} />
                    <View style={styles.playbackControls}>
                        <Button variant="outline" size="icon" onPress={handleDeleteRecording} style={styles.controlButton}>
                ...
                <View>
                    ...
                    {isRecording ? (
                        <View style={styles.recordingStatus}>
                            <View style={styles.recordingIndicator}>
                                ...
                                <TextX variant="description" style={{ color: redColor, marginLeft: 8 }}>
                                    Recording
                                </TextX>
                            </View>
                        </View>
                    ...
                    {showTimer && (
                        <View style={styles.timerContainer}>
                            ...
                            {maxDuration && (
                                <TextX variant="description" style={{ color: mutedColor }}>
