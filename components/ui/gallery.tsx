...
export const useImageZoom = ({ enableZoom, onSetCanSwipe, shouldReset = false }: UseImageZoomProps) => {
    ...
    }, [scale, translateX, translateY, savedScale, savedTranslateX, savedTranslateY, onSetCanSwipe, panGestureEnabled]);
    ...
    const constrainTranslation = useCallback((newScale: number, newTranslateX: number, newTranslateY: number) => {
        'worklet';
...
const FullscreenImage = memo(({ item, index, selectedIndex, enableZoom, onSetCanSwipe }: FullscreenImageProps) => {
    ...
    return (
        <View
            ...
            {enableZoom ? (
                ...
                <Animated.View style={styles.imageContainer}>
                    <AnimatedImage source={{ uri: item.uri }} style={[styles.fullscreenImage, animatedImageStyle]} contentFit="contain" />
                </Animated.View>
            )}
        </View>
    );
...
export function Gallery({
    ...
    const renderGalleryItem = useCallback(
                ...
                {(showTitles || showDescriptions) && (
                    <View style={[styles.itemInfo]}>
                        {showTitles && item.title && (
                            <TextX variant="subtitle" numberOfLines={1} style={{ color: textColor }}>
                                {item.title}
                            </TextX>
                        )}
                        {showDescriptions && item.description && (
                            <TextX variant="body" numberOfLines={2} style={{ color: mutedColor }}>
                                {item.description}
                            </TextX>
                        )}
                    </View>
        ...
        [itemWidth, aspectRatio, borderRadius, handleItemPress, renderCustomOverlay, showTitles, showDescriptions, textColor, mutedColor],
    );
            ...
            <View style={styles.fullscreenControls} pointerEvents="box-none">
                ...
                <View style={[styles.topControls, { backgroundColor }]}>
                    <View style={styles.topRightControls}>
                        ...
                        <ButtonX size="sm" variant="ghost" onPress={handleShare}>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item, index }) => (
                            <Pressable
                                ...
                                <Image source={{ uri: item.thumbnail || item.uri }} style={styles.thumbnailImage} contentFit="cover" />
                            </Pressable>
                        )}
    ...
    if (items.length === 0) {
        return (
            <View style={[styles.emptyState]}>
                <TextX variant="subtitle" style={{ color: mutedColor }}>
