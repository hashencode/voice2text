...
export const Image = forwardRef<ExpoImage, ImageProps>(
    (
        ...
        const imageStyles = [{ width: '100%', height: '100%', borderRadius }, style].filter(Boolean) as ExpoImageProps['style'];

        const containerStyles = [styles.container, containerDimensions, { borderRadius, backgroundColor }, containerStyle];
            ...
            <View style={containerStyles}>
                ...
                {isLoading && showLoadingIndicator && (
                    <View style={styles.overlay}>
                        <ActivityIndicator size={loadingIndicatorSize} color={loadingIndicatorColor || primaryColor} />
                    </View>
                )}
                ...
                {hasError && showErrorFallback && (
                    <View style={[styles.overlay, styles.errorContainer]}>
                        <TextX variant="body" style={[styles.errorText, { color: textColor }]} numberOfLines={2}>
                            {errorFallbackText}
                        </TextX>
                    </View>
                )}
