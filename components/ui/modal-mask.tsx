...
export function ModalMask({
  ...
  const resolvedContentDuration = contentTransitionDuration ?? contentFadeDuration;
  ...
  React.useEffect(() => {
    ...
    const shouldWaitContentAnimation = resolvedContentDuration > fadeDuration;
  ...
  const contentFadeStyle = useAnimatedStyle(
    () => ({
      opacity:
        contentTransitionPreset === 'none'
          ? 1
          : interpolate(contentProgress.value, [0, 1], [0, 1]),
      transform: (() => {
    ...
    [
      ...
      resolvedTransitionDistance,
      contentTransitionScaleFrom,
    ]
  );
  ...
  const maskNode = renderMask
    ? renderMask({ onPressMask, defaultMask })
    : defaultMask;
  ...
  return (
    ...
    >
      <View style={styles.container}>
        {maskNode}
        <Animated.View
          style={[styles.contentLayer, contentFadeStyle]}
