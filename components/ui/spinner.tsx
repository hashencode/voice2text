...
const AnimatedDot = React.memo(({ anim, color, size, style }: AnimatedShapeProps) => {
    ...
    return <Animated.View style={[style, { width: size, height: size, backgroundColor: color }, animatedStyle]} />;
});
...
const AnimatedBar = React.memo(({ anim, color, size, style }: AnimatedShapeProps) => {
    ...
    return <Animated.View style={[style, { width: size / 6, height: size, backgroundColor: color }, animatedStyle]} />;
});

// Main Spinner Component
export function Spinner({ size = 'default', variant = 'default', label, showLabel = false, style, color, speed = 'normal' }: SpinnerProps) {
    ...
    const barsAnims = useMemo(() => [barAnim1, barAnim2, barAnim3, barAnim4], [barAnim1, barAnim2, barAnim3, barAnim4]);
    // --- END FIX ---
    ...
    useEffect(() => {
        if (variant === 'circle') {
            rotate.value = withRepeat(withTiming(360, { duration: animationDuration, easing: Easing.linear }), -1);
        } else {
    ...
    useEffect(() => {
        if (variant === 'pulse') {
            pulse.value = withRepeat(
                withSequence(withTiming(1.3, { duration: animationDuration / 2 }), withTiming(1, { duration: animationDuration / 2 })),
                -1,
                true,
            );
        } else {
        ...
        if (variant === 'dots') {
            dotsAnims.forEach((anim, index) => {
                anim.value = withRepeat(
                    withSequence(
                        withDelay(index * (animationDuration / 6), withTiming(1, { duration: animationDuration / 3 })),
    ...
    useEffect(() => {
        if (variant === 'bars') {
            barsAnims.forEach((anim, index) => {
                anim.value = withRepeat(
                    withSequence(
                        withDelay(index * (animationDuration / 8), withTiming(1, { duration: animationDuration / 4 })),
    ...
    const renderSpinner = () => {
        switch (variant) {
            ...
            case 'circle':
                return (
                    <Animated.View style={[styles.customSpinner, { width: config.size, height: config.size }, animatedCircleStyle]}>
                ...
                return (
                    <View style={[styles.dotsContainer, { gap: config.size / 4 }]}>
                        {dotsAnims.map((anim, index) => (
                            <AnimatedDot key={index} anim={anim} color={spinnerColor} size={config.size / 3} style={styles.dot} />
                        ))}
                    </View>
                );
                ...
                return (
                    <View style={[styles.barsContainer, { gap: config.size / 6 }]}>
                        {barsAnims.map((anim, index) => (
                            <AnimatedBar key={index} anim={anim} color={spinnerColor} size={config.size} style={styles.bar} />
                        ))}
                    </View>
                );
...
export function InlineLoader({ size = 'sm', variant = 'default', color }: Omit<SpinnerProps, 'label' | 'showLabel'>) {
...
export function ButtonSpinner({ size = 'sm', variant = 'default', color }: Omit<SpinnerProps, 'label' | 'showLabel'>) {
    ...
    return <Spinner size={size} variant={variant} color={color || primaryForegroundColor} style={styles.buttonSpinner} />;
}
